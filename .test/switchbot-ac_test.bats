...
@test "no args: show usage and exit 1" {
  run "$ROOT/.bin/switchbot-ac"
  [ "$status" -eq 1 ]
  [[ "$output" == *"Usage"* ]]
}

@test "with device and action: call API" {
  run "$ROOT/.bin/switchbot-ac" -d device123 -a "power=on"
  [ "$status" -eq 0 ]
  [[ "$output" == *'"status":"ok"'* ]]
  grep -q "https://api.switch-bot.com/v1.1/devices/device123/commands" "$WGET_ARGS_FILE"
  grep -q -- '--post-data={"command": "setAll","parameter": "power=on","commandType": "command"}' "$WGET_ARGS_FILE"
}

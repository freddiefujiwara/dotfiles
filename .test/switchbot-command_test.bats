...
@test "no args: show usage and exit 1" {
  run "$ROOT/.bin/switchbot-command"
  [ "$status" -eq 1 ]
  [[ "$output" == *"Usage"* ]]
}

@test "with device and command: call API" {
  run "$ROOT/.bin/switchbot-command" -d device123 -c press
  [ "$status" -eq 0 ]
  [[ "$output" == *'"status":"ok"'* ]]
  grep -q "https://api.switch-bot.com/v1.1/devices/device123/commands" "$WGET_ARGS_FILE"
  grep -q -- '--post-data={"command": "press","parameter": "default","commandType": "command"}' "$WGET_ARGS_FILE"
}
